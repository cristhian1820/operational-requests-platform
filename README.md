...
El runner debe soportar Docker-in-Docker en modo privilegiado para los jobs de Compose y construcción de imágenes. En Karate, Compose se levanta en el daemon DinD, se obtiene el nombre real de la red con `docker inspect`, y tanto los health checks como Maven se ejecutan en contenedores unidos a esa red: `requests-service:8081`, `indicators-service:8082`, `shell:8080`, `requests-mfe:8080` y `keycloak:8080`; no se usa `localhost`. El contenedor Maven temporal se elimina y `after_script` ejecuta `docker compose down --remove-orphans` sin borrar volúmenes externos. Variables opcionales para el build frontend son `KEYCLOAK_URL`, `KEYCLOAK_REALM`, `KEYCLOAK_CLIENT_ID`, `REQUESTS_API_URL`, `INDICATORS_API_URL` y `REQUESTS_MFE_URL`; los valores sensibles estándar de Registry (`CI_REGISTRY_USER`, `CI_REGISTRY_PASSWORD`) los proporciona GitLab y no se escriben en el YAML. No se requiere `.env` privado en CI: Karate usa `.env.example` y sus credenciales locales ficticias. Para validar localmente el pipeline se puede ejecutar `python -m pip install PyYAML==6.0.2` y `python tests/ci/validate-gitlab-ci.py`; CI Lint de GitLab requiere un servidor/token y no se afirma haberlo ejecutado.
