...
public class RequestsController {
 ...
 public PageResult<SolicitudSummary> list(@RequestParam(name="page",defaultValue="0") @Min(0) int page,@RequestParam(name="size",defaultValue="20") @Min(1) @Max(100) int size,
   @RequestParam(name="estado",required=false) EstadoSolicitud estado,@RequestParam(name="categoriaId",required=false) UUID categoriaId,@RequestParam(name="prioridad",required=false) Prioridad prioridad,@AuthenticationPrincipal Jwt jwt){
 ...
 @Operation(summary="Consulta el detalle, observaciones e historial") public SolicitudDetail detail(@PathVariable(name="id") UUID id,@AuthenticationPrincipal Jwt jwt){return useCase.detail(id,actors.from(jwt));}
 ...
 public SolicitudDetail assign(@PathVariable(name="id") UUID id,@AuthenticationPrincipal Jwt jwt,HttpServletRequest request){return useCase.assign(id,actors.from(jwt),correlation(request));}
 @PostMapping("/solicitudes/{id}/observaciones") @PreAuthorize("hasRole('ANALISTA')")
 @Operation(summary="Agrega una observación como analista asignado") public SolicitudDetail observe(@PathVariable(name="id") UUID id,@Valid @RequestBody ObservationRequest body,@AuthenticationPrincipal Jwt jwt){return useCase.observe(id,body.texto(),actors.from(jwt));}
 ...
 public SolicitudDetail transition(@PathVariable(name="id") UUID id,@Valid @RequestBody TransitionRequest body,@AuthenticationPrincipal Jwt jwt,HttpServletRequest request){return useCase.transition(new TransicionarSolicitudCommand(id,body.estadoDestino(),body.motivo(),body.observacion(),actors.from(jwt),correlation(request)));}
